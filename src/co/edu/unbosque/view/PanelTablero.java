...
public class PanelTablero extends JPanel {
    ...
    private final Color COLOR_FONDO_VACIO = new Color(30, 32, 33); // Gris oscuro industrial (despega los logos
                                                                   // negros/oscuros)
     ...
     * en el centro ubica el mapa dinámico y abajo prepara los controles de
     * movimiento.
     */
    public PanelTablero() {
    ...
    public JButton getBtnMoverArriba() {
        return btnMoverArriba;
    }

    public JButton getBtnMoverAbajo() {
        return btnMoverAbajo;
    }

    public JButton getBtnMoverIzquierda() {
        return btnMoverIzquierda;
    }

    public JButton getBtnMoverDerecha() {
        return btnMoverDerecha;
    }
    ...
    public void actualizarTablero(Casilla[][] matriz) {
        ...
        for (int i = 0; i < matriz.length; i++) {
            for (int j = 0; j < matriz[i].length; j++) {
                ...
                switch (tipo) {
                    case "SCRIPT":
                        celdas[i][j].setIcon(imgHacker);
                        break;
                    case "PAQUETE":
                        celdas[i][j].setIcon(imgPaquete);
                        break;
                    case "FIREWALL":
                        celdas[i][j].setIcon(imgFirewall);
                        break;
                    case "ANTIVIRUS":
                        celdas[i][j].setIcon(imgAntivirus);
                        break;
                    case "ESCANER":
                        celdas[i][j].setIcon(imgEscaner);
                        break;
                    case "NODO":
                        celdas[i][j].setIcon(imgNodo);
                        break;
                    case "PUERTO":
                        celdas[i][j].setIcon(imgPuerto);
                        break;
                    default:
